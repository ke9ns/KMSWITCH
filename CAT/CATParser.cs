...
namespace KMSWITCH
{
	...
	public class CATParser
	{
       ...
       public Int32 Verbose_Error_Code
		...
		public CATParser(Form1 c)
		{
			form1 = c;
			cmdlist = new CATCommands(form1,this);
			GetCATData();
		}
		...
		private void GetCATData()
		{
			...
			try
			{
             //   string file_name2 = AppDataPath + "ke9ns8.dat"; // save data for my mods
                ...
                doc.Load("C:\\KMSWITCH\\CATStructs.xml");
			}
			...
			if(goodcmd)
			{
				switch(prefix)
				{
					...
					case "EX":
						break;
					case "FA":
                        Debug.WriteLine("testing0 = " + suffix); //ke9ns: when I receive a FA it should be FA00014259000 (vfoa freq from powerSDR)

						int vfo = 0;
                       bool success = int.TryParse(suffix, out vfo);
						if (success)
						{
							Debug.WriteLine("testing1 " + vfo);
                            
                           form1.NewVFOA = vfo;
						}

                      //  rtncmd = cmdlist.FA(suffix);
						//Debug.WriteLine("testing1..." + rtncmd);
						break;
					case "FB":
