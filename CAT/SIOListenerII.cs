...
{	
	public class SIOListenerII
	{
		...
		public SIOListenerII(Form1 c)
		{
			form1 = c;
			form1.Activated += new EventHandler(console_Activated);
			form1.Closing += new System.ComponentModel.CancelEventHandler(console_Closing);
         ...
         //   Debug.WriteLine("siolisten enable cat now0");

            if ( form1.CATEnabled )  // if CAT is on, fire it up 
			{
              //  Debug.WriteLine("siolisten enable cat now1");

                try
                {
                    Debug.WriteLine("siolisten enable cat now3");
				...
				{					
					...
					if ( form1.setupForm != null ) 
					{ 
					//	form1.setupForm.copyCATPropsToDialogVars(); // need to make sure the props on the setup page get reset 
					}
...
//		}
		...
		public void enableCAT() 
		{
            ...
            int port_num = form1.CATPort;
            Debug.WriteLine("==============CAT PORT OPEN " + port_num);

           // SIO = new SDRSerialPort(port_num);
		//	SIO.setCommParms(form1.CATBaudRate, form1.CATParity, form1.CATDataBits, form1.CATStopBits,form1.CATHandshake);

       form1.sDRSerialPort =  SIO = new SDRSerialPort(port_num, form1.CATBaudRate,form1.CATParity,form1.CATDataBits,form1.CATStopBits, form1.CATHandshake);
...
//		private int SIOMonitorCount = 0;
		...
		{ 
#if(!DBG_PRINT) 
			Form1.dbgWriteLine("SIOListener: " + s); 
#endif
		}

		// Called when the form1 is activated for the first time.  
...
//			SIOMonitor.Enabled = true;		// start or restart the timer
			...
			Debug.WriteLine("SIOLISTENER commbuffer " + CommBuffer.ToString());
